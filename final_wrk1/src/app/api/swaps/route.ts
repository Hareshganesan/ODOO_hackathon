...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get skill information separately
    const enrichedSwapRequests = await Promise.all(
      swapRequests.map(async (swap) => {
        const skillOffered = await prisma.userSkill.findUnique({
          where: { id: swap.skillOfferedId },
          include: {
            skill: {
              select: {
                id: true,
        ...
        const skillWanted = await prisma.userSkill.findUnique({
          where: { id: swap.skillWantedId },
          include: {
            skill: {
              select: {
                id: true,
